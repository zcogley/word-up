...
function wordElem(wordSubmission) {
  ...
  if (wordSubmission.hasOwnProperty("exists")) {
    var badge = $("<span></span>")
      .text(wordSubmission.exists ? wordScore(wordSubmission.word) : "X")
      .attr("class", "tag tag-sm")
      .addClass(wordSubmission.exists ? "tag-success" : "tag-danger");
    elem.append(badge);
  }
  else {
...
function currentScore() {
  return model.wordSubmissions.map(function(wordSubmission) {
    return wordSubmission.exists ? wordScore(wordSubmission.word) : 0;
  }).reduce(add, 0);
}
